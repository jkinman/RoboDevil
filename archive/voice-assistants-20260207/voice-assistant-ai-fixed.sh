...
# Send message to OpenClaw and wait for AI response
send_and_wait() {
    ...
    send_result=$(curl -s -X POST http://$IPC_HOST:$IPC_PORT/responses \
        -H "Content-Type: application/json" \
        -d "{
            \"text\": \"$user_message\",
            \"source\": \"voice\",
    ...
    while [ $attempts -lt $max_attempts ]; do
        ...
        response_data=$(curl -s "http://$IPC_HOST:$IPC_PORT/ai-response?session=$SESSION_ID" 2>/dev/null)
...
record_audio() {
    # Use pw-record with longer timeout (8 seconds)
    # Records continuously until timeout or silence for extended period
    timeout 8 pw-record --rate 16000 --channels 1 --format s16 "$AUDIO_FILE" 2>/dev/null
}
